...
)
{
    ...
    if (convval->category == RIR_VALUE_CONSTANT) {
        ...
        struct rir_block *taken_block = rir_block_create_from_ast(
            NULL,
            BLOCK_POSITION_IF,
            ctx
        );
        ...
        struct rir_block *fallthrough_block = rir_block_create_from_ast(
            NULL,
            BLOCK_POSITION_IF,
            ctx
        );
        ...
        struct rir_block *after_block = rir_block_create_from_ast(
            NULL,
            BLOCK_POSITION_NORMAL,
            ctx
        );
        if (!after_block) {
            return NULL;
...
bool rir_process_convertcall(const struct ast_node *n, struct rir_ctx *ctx)
{
    ...
    struct rir_object *obj = rir_convert_create_obj_maybeadd(
        argexprval,
        rir_type_create_from_type(ast_node_get_type(n), RIR_LOC_CONVERSION, ctx),
        RIRPOS_AST,
        ctx
    );
    if (!obj) {
