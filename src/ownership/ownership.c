...
static inline bool ow_ctx_add_new(struct rir_value *n, struct rir_object *obj)
{
    RF_ASSERT(g_ow_ctx, "No global ownership context exists");
    struct ow_graph *v = ow_graph_create(n, obj);
    if (!v) {
        return false;
    }
...
void ow_ctx_check(const struct rir_value *v, struct rir_expression *expr)
{
    RF_ASSERT(g_ow_ctx, "No global ownership context exists");
    struct ow_graph **graph;
    darray_foreach(graph, g_ow_ctx->graphs) {
        if (ow_graph_check_or_add_val(*graph, rir_expression_to_obj(expr))) {
...
void ow_ctx_check_expr(struct rir_expression *expr)
{
    ...
    case RIR_EXPRESSION_OBJMEMBERAT:
        ow_ctx_check(expr->objmemberat.objmemory, expr);
        break;
    case RIR_EXPRESSION_UNIONMEMBERAT:
        ow_ctx_check(expr->unionmemberat.unimemory, expr);
        break;
    case RIR_EXPRESSION_READ:
        ow_ctx_check(expr->read.memory, expr);
        break;
    case RIR_EXPRESSION_WRITE:
        ow_ctx_check(expr->write.memory, expr);
        ow_ctx_check(expr->write.writeval, expr);
        break;
    case RIR_EXPRESSION_CONVERT:
        ow_ctx_check(expr->convert.val, expr);
        break;
    case RIR_EXPRESSION_ADD:
    case RIR_EXPRESSION_SUB:
    case RIR_EXPRESSION_MUL:
    case RIR_EXPRESSION_DIV:
        ow_ctx_check(expr->binaryop.a, expr);
        ow_ctx_check(expr->binaryop.b, expr);
        break;
    ...
    darray_foreach(b, f->blocks) {
        ...
        rf_ilist_for_each(&(*b)->expressions, expr, ln) {
            ...
            if (expr->type == RIR_EXPRESSION_ALLOCA) {
                if (!ow_ctx_add_new(&expr->val, rir_expression_to_obj(expr))) {
                    return false;
                }
...
#if RF_HAVE_GRAPHVIZ
    // for now just always create the SVG graphs
