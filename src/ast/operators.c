...
{
    RF_ASSERT(
        token_is_binaryop(tok),
        "Requested binary op type from non binaryop token type \""RFS_PF"\"",
...
{
    RF_ASSERT(
        token_is_unaryop(tok),
        "Requested unary op type from non-unary token type \""RFS_PF"\"",
        RFS_PA(tokentype_to_str(tok->type))
    );
